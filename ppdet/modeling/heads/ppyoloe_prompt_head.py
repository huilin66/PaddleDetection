...
class CrossAttention(nn.Layer):
    ...
    def forward(self, x1, x2):
        attn_weights = paddle.einsum('bnc,bmc->bnm', x1, x2)/ (self.dim ** 0.5)
...
#
#     def forward(self, x1, x2):
#         attn_weights = x1 @ x2.transpose((1, 0)) / (self.dim ** 0.5)
#         attn_weights = F.softmax(attn_weights, axis=-1)
#         return attn_weights
...
class PromptHead(nn.Layer):
    ...
    def forward(self, features, labels, scores):
        ...
        att_weight = self.att(features_flat, features_prompt)
        return att_weight


class SupPromptHead(nn.Layer):
    __shared__ = [
        'num_classes'
    ]

    def __init__(self, num_classes=80, temperature=2.5, sample_num=4096, thresh=0.75):
        super(SupPromptHead, self).__init__()
        self.num_classes = num_classes
        self.temperature = temperature
        self.sample_num = sample_num
        self.thresh = thresh
    ...
    def forward(self, features, labels):
        ...
        positive_mask = (labels < self.num_classes)
        positive_features, positive_labels = features[positive_mask], labels[positive_mask]


        # negative_mask = (labels == self.num_classes)
        # negative_features, negative_labels = features[negative_mask], labels[negative_mask]
        #
        #
        # N = negative_features.shape[0]
        # S = self.sample_num - positive_mask.sum()
        ...
        similarity = (paddle.matmul(positive_features, features.T) / self.temperature)
        # positive_features
        # if len(labels.shape) == 1:
        #     labels = labels.reshape([-1, 1])
        # label_mask = paddle.equal(labels, labels.T).detach()
...
@register
class ContrastHead0(nn.Layer):
    __shared__ = [
        'num_classes'
    ]

    def __init__(self, num_classes=80, temperature=100, sample_num=2048, thresh=0.75):
        super(ContrastHead0, self).__init__()
        self.num_classes = num_classes
        self.temperature = temperature
        self.sample_num = sample_num
        self.thresh = thresh
...
@register
class PPYOLOEPromptHead0(PPYOLOEHead):
    __shared__ = [
        'num_classes', 'eval_size', 'trt', 'exclude_nms',
        'exclude_post_process', 'use_shared_conv', 'for_distill'
    ]
    ...
    def __init__(self,
        ...
        self.contrast_loss = contrast_loss
        self.contrast_encoder = nn.LayerList()
        self.prompt_heads = nn.LayerList()
        sample_nums = [2048, 2048, 2048]
        for idx, in_c in enumerate(self.in_channels):
            self.contrast_encoder.append(nn.Conv2D(in_c, 128, 3, padding=1))
            self.prompt_heads.append(ContrastHead0(num_classes=self.num_classes, temperature=100, sample_num=sample_nums[idx], thresh=0.75))
        self._init_contrast_encoder()
    ...
    def forward_train(self, feats, targets, aux_pred=None):
        ...
        for i, feat in enumerate(feats):
            ...
            avg_feat = F.adaptive_avg_pool2d(feat, (1, 1))
            stem_feature = self.stem_cls[i](feat, avg_feat)
            contrast_logit = self.contrast_encoder[i](stem_feature + feat)
            contrast_logit = contrast_logit.flatten(2).transpose([0, 2, 1])
            similarity, loss = self.prompt_heads[i](contrast_logit.reshape([-1, contrast_logit.shape[-1]]), \
                                               assigned_labels.reshape([-1]), assigned_scores.max(-1).reshape([-1]))
            loss_contrast.append(loss)
            cls_logit = self.pred_cls[i](stem_feature + feat)
            reg_distri = self.pred_reg[i](self.stem_reg[i](feat, avg_feat))
        ...
        return self.get_loss([
            cls_score_list, reg_distri_list, contrast_encoder_list, anchors,
            anchor_points, num_anchors_list, stride_tensor, loss_contrast
        ], targets)

    def get_loss(self, head_outs, gt_meta):
        pred_scores, pred_distri, pred_contrast_encoder, anchors,\
        anchor_points, num_anchors_list, stride_tensor, loss_contrast = head_outs
...
class PromptHead3(nn.Layer):
    __shared__ = [
        'num_classes'
    ]

    def __init__(self, in_chs, num_classes=7, ):
        super(PromptHead3, self).__init__()
        self.num_classes = num_classes
        self.att = CrossAttention(in_chs)
    ...
    def forward(self, features, labels, scores):
        ...
        n, c, h, w = features.shape
        features_flat = features.transpose((0, 2, 3, 1)).reshape((n, -1, c))

        # features_flat = features.reshape((n, c, -1))
        # features_flat = features_flat.transpose((0, 2, 1))
        # features_prompt = features_flat.reshape((-1, c))

        features_positive = features_flat[labels<self.num_classes]
        if features_positive.shape[0] == 0:
            return 0
        else:
            features_positive = features_positive.unsqueeze(0).repeat_interleave(repeats=n, axis=0)
        labels_positive = labels[labels<self.num_classes]

        cos_sim = self.att(features_positive, features_flat)
        # cos_sim = F.cosine_similarity(features_positive, features_prompt, axis=1)
        sim_weight = cos_sim.unsqueeze(1).repeat_interleave(repeats=self.num_classes, axis=1)
        sim_weight = paddle.sum(sim_weight, axis=2).reshape((n, -1, h, w ))
        return sim_weight

@register
class PPYOLOEPromptHead3(PPYOLOEHead):
    __shared__ = [
        'num_classes', 'eval_size', 'trt', 'exclude_nms',
        'exclude_post_process', 'use_shared_conv', 'for_distill'
    ]
    ...
    def __init__(self,
        ...
        for in_c in self.in_channels:
            self.prompt_head.append(PromptHead3(in_c, self.num_classes))
            self.stem_cls.append(ESEAttn(in_c, act=act, attn_conv=attn_conv))
        self._init_contrast_encoder()
...
class SupPromptHead4(nn.Layer):
    __shared__ = [
        'num_classes'
    ]

    def __init__(self, num_classes=80, temperature=2.5, sample_num=4096, thresh=0.75):
        super(SupPromptHead4, self).__init__()
        self.num_classes = num_classes
        self.temperature = temperature
        self.sample_num = sample_num
        self.thresh = thresh
...
@register
class PPYOLOEPromptHead4(PPYOLOEHead):
    __shared__ = [
        'num_classes', 'eval_size', 'trt', 'exclude_nms',
        'exclude_post_process', 'use_shared_conv', 'for_distill'
    ]
    ...
    def __init__(self,
        ...
        self.contrast_encoder = nn.LayerList()
        self.prompt_heads = nn.LayerList()
        sample_nums = [100, 400, 1600]
        for idx, in_c in enumerate(self.in_channels):
            self.contrast_encoder.append(nn.Conv2D(in_c, 128, 3, padding=1))
            self.prompt_heads.append(SupPromptHead(num_classes=self.num_classes, temperature=100, sample_num=sample_nums[idx], thresh=0.75))
        self._init_contrast_encoder()
