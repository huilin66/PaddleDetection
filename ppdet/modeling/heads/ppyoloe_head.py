...
class ESEAttn(nn.Layer):
    def __init__(self, feat_channels, out_channels=None, act='swish', attn_conv='convbn'):
        super(ESEAttn, self).__init__()
        if out_channels is None:
            out_channels = feat_channels
        self.fc = nn.Conv2D(feat_channels, out_channels, 1)
        if attn_conv == 'convbn':
            self.conv = ConvBNLayer(feat_channels, out_channels, 1, act=act)
        elif attn_conv == 'repvgg':
            self.conv = RepVggBlock(feat_channels, out_channels, act=act)
        else:
            self.conv = None
        self._init_weights()
